...
// Comet-PTM start
   ...
   int bUseDeltaClosest;         // 0=no (default), 1=yes
   int bPrintXcorrProfiles;     // 0=no (default), 1=yes
   int iCandNumber;              // ignored if use_delta_closest 0, default 1
   int bUseXcorrCorr;            // 0=no (default), 1=yes
// Comet-PTM end
...
// Comet-PTM start
      ...
      bUseDeltaClosest = a.bUseDeltaClosest;
      bPrintXcorrProfiles = a.bPrintXcorrProfiles;
      iCandNumber = a.iCandNumber;
      bUseXcorrCorr = a.bUseXcorrCorr;
// Comet-PTM end
...
struct Results
{
   ...
   float  fXcorr;
   float  fXcorrCorr; // ReCom //TODO put here
   float  fXcorr_nosort; // ReCom //TODO remove from here
   float  fNonModXcorr;
   int    iLenPeptide;
   int    iRankSp;
   int    iMatchedIons;
...
// Comet-PTM start
   ...
   int iStartPos; // ReCom
   int iEndPos; // ReCom
   char szXcorrType[WIDTH_REFERENCE]; // ReCom // TODO
   char szXcorrProfileClosest[MAX_PEPTIDE_LEN*12]; // ReCom // TODO
   char szXcorrProfileBClosest[MAX_PEPTIDE_LEN*12]; // ReCom // TODO
   char szXcorrProfileYClosest[MAX_PEPTIDE_LEN*12]; // ReCom // TODO
   vector<char> vcProteinSeq; // ReCom
   std::vector<std::vector<std::vector<std::vector<double> > > > vctXcorrs; // ReCom
   //vector<bool> vbDuplFragmentClosest; // ReCom
// Comet-PTM end
};
...
struct ToleranceParams
{
   ...
   ToleranceParams& operator=(ToleranceParams& a)
   {
      ...
      dDeltaTolerance = a.dDeltaTolerance;
...
struct StaticParams
{
   ...
   vector<double>  vectorDeltaPeaks;
   int             iElapseTime;
   char            szDate[32];
   Options         options;
   DBInfo          databaseInfo;
...
// Comet-PTM start
      ...
      options.bUseDeltaClosest = 0;
      options.iCandNumber = 1;
      options.bUseXcorrCorr = 0;
// Comet-PTM end
   ...
   StaticParams& operator=(StaticParams& a)
   {
      ...
      vectorDeltaPeaks = a.vectorDeltaPeaks;
...
struct Query
{
   ...
   // ReCom: Array data for XCorr rescoring
   bool *pbDuplFragment; // TODO
   unsigned int  uiBinnedIonMasses[MAX_FRAGMENT_CHARGE+1][9][MAX_PEPTIDE_LEN];
