...
// Comet-PTM start
   ...
   GetParamValue("deltamass_peaks", g_staticParams.vectorDeltaPeaks);
   GetParamValue("deltamass_peaks_tolerance", g_staticParams.tolerances.dDeltaTolerance);
   GetParamValue("use_delta_closest", g_staticParams.options.bUseDeltaClosest);
   GetParamValue("print_xcorr_profiles", g_staticParams.options.bPrintXcorrProfiles);
   GetParamValue("cand_number", g_staticParams.options.iCandNumber);
   GetParamValue("use_xcorr_corr", g_staticParams.options.bUseXcorrCorr);
// Comet-PTM end
...
bool CometSearchManager::DoSearch()
{
   ...
   {
      sprintf(szOut, "ReCom (Comet version \"%s\")", comet_version);
...
//             if (g_staticParams.options.bUseDeltaClosest == 1)
//             {
//                if (!g_staticParams.options.bOutputSqtStream && !g_staticParams.bIndexDb)
//                {
//                   logout("     - Rescoring:");
...
//                }
// 
//                g_cometStatus.SetStatusMsg(string("Rescoring ..."));
// 
//                // Rescore selected candidates (from parameter "cand_number")
//                bSucceeded = CometRescore::Rescore(g_staticParams.options.iNumThreads, g_staticParams.options.iNumThreads); // TODO move to postanalysis?
//                if (!bSucceeded)
//                   goto cleanup_results;
// 
// #ifdef PERF_DEBUG
...
//                   time(&tPostAnalysisEndTime);
//                   strftime(szTimeBuffer, 26, "%m/%d/%Y, %I:%M:%S %p", localtime(&tPostAnalysisEndTime));
//                   sprintf(szOut, " - End Rescoring:  %s\n", szTimeBuffer);
//                   logout(szOut);
//                   int iElapsedTime=(int)difftime(tPostAnalysisEndTime, tPostAnalysisStartTime);
//                   sprintf(szOut, " - Time spent in Rescoring:  %d seconds\n", iElapsedTime);
//                   logout(szOut);
//                   fflush(stdout);
//                }
// #endif
