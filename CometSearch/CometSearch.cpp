                            ...
                            int iPercentEnd)
{
   ...
   else
   {
      ...
      {
         char szOut[128];
         sprintf(szOut, "     - Search progress [end @ %i\%]: ", iPercentEnd);
         logout(szOut);
            ...
            while (iWhichQuery < (int)g_pvQuery.size())
            {
               ...
               if (CheckMassMatch(iWhichQuery, dCalcPepMass))
               {
                  ...
                  if (bFirstTimeThroughLoopForPeptide)
                  {
                     ...
                     g_pvQuery.at(iWhichQuery)->pbDuplFragment = pbDuplFragment; // TODO // sorting before rescoring?
                     for (ctCharge=0; ctCharge<6; ctCharge++)
                     {
                        for (ctIonSeries=0; ctIonSeries<9; ctIonSeries++)
                        {
                           for (ctLen=0; ctLen<64; ctLen++)
                           {
                              g_pvQuery.at(iWhichQuery)->uiBinnedIonMasses[ctCharge][ctIonSeries][ctLen] = _uiBinnedIonMasses[ctCharge][ctIonSeries][ctLen];
//                               if (_uiBinnedIonMasses[ctCharge][ctIonSeries][ctLen] != g_pvQuery.at(iWhichQuery)->uiBinnedIonMasses[ctCharge][ctIonSeries][ctLen])
//                               {
//                                  char szOut[128];
//                                  sprintf(szOut, "Coor:  %i, %i, %i\n", ctCharge, ctIonSeries, ctLen);
//                                  logout(szOut);
//                                  fflush(stdout);
//                                  sprintf(szOut, "Value:  %i, %i\n", _uiBinnedIonMasses[ctCharge][ctIonSeries][ctLen], g_pvQuery.at(iWhichQuery)->uiBinnedIonMasses[ctCharge][ctIonSeries][ctLen]);
//                                  logout(szOut);
//                                  fflush(stdout);
//                               }
                           }
...
//                      if (_uiBinnedIonMasses != g_pvQuery.at(iWhichQuery)->uiBinnedIonMasses)
//                      {
//                         logout("DIFF");
//                         fflush(stdout);
//                      }
                  ...
                  double deltaXcorrMass;           // Comet-PTM

                  if (g_staticParams.options.bUseDeltaXcorr) // Comet-PTM
                  {
                     // Comet-PTM: don´t calculate modified xcorr when delta mass not near 0
                     deltaXcorrMass = g_pvQuery.at(iWhichQuery)->_pepMassInfo.dExpPepMass - dCalcPepMass;
                     bDeltaXcorrSearch = g_staticParams.options.bUseDeltaXcorr
                        && g_staticParams.options.bDontCalcPseudoNonMod
                        && (deltaXcorrMass < -g_staticParams.options.dDeltaInnerTol
                              || deltaXcorrMass > g_staticParams.options.dDeltaInnerTol);
                  }

                  if (!g_staticParams.variableModParameters.bRequireVarMod && !bDeltaXcorrSearch)
                  {
                     XcorrScore(szProteinSeq, iStartPos, iEndPos, iStartPos, iEndPos, false,
                           dCalcPepMass, false, iWhichQuery, iLenPeptide, piVarModSites, &dbe, pbDuplFragment);

                     if (g_staticParams.options.iDecoySearch)
                     {
                        XcorrScore(szDecoyPeptide, iStartPos, iEndPos, 1, iLenPeptide, false,
                              dCalcPepMass, true, iWhichQuery, iLenPeptide, piVarModSites, &dbe, pbDuplFragment);
                     }
                  }
...
// SPIROS END comet-ptm
               ...
               // ReCom: Save array for XCorr rescoring // TODO
               //g_pvQuery.at(iWhichQuery)->pbDuplFragment = pbDuplFragment;
                                     ...
                                     int iEndPos)
{
   ...
   if (dCalcPepMass >= g_massRange.dMinMass
         ...
         && iLength >= g_staticParams.options.iPeptideLengthMin
         && (g_staticParams.options.iPeptideLengthMax == 0 || (iLength <= g_staticParams.options.iPeptideLengthMax)))
   {
                                     ...
                                     int iEndPos)
{
   if (!g_staticParams.options.bNoEnzymeSelected)
   {
      ...
      if (g_staticParams.enzymeInformation.szSearchEnzyme2Name[0] != '\0') // if second enzyme specified
      {
         if (!bBeginCleavage)
         {
      ...
      for (i=iStartPos; i<=iEndPos; i++)
      {
         ...
         // This will analyze possible missed cleavage for second enzyme only if there's not a missed cleavage for first enzyme.
         // This will not support the non-existent case where a residue can be a missed cleavage both n- and c-term at same time.
         if (!bBreakPoint && g_staticParams.enzymeInformation.szSearchEnzyme2Name[0] != '\0')
         {
            bBreakPoint = strchr(g_staticParams.enzymeInformation.szSearchEnzyme2BreakAA, szProteinSeq[i - g_staticParams.enzymeInformation.iSearchEnzyme2OffSet + 1])
               && !strchr(g_staticParams.enzymeInformation.szSearchEnzyme2NoBreakAA, szProteinSeq[i - g_staticParams.enzymeInformation.iSearchEnzyme2OffSet + 2]);
         }
                                          ...
                                          int iStartPos)
{
   if (!g_staticParams.options.bNoEnzymeSelected)
   {
      ...
      if (!bBeginCleavage && g_staticParams.enzymeInformation.szSearchEnzyme2Name[0] != '\0') // if second enzyme specified
      {
         bBeginCleavage = strchr(g_staticParams.enzymeInformation.szSearchEnzyme2BreakAA, szProteinSeq[iStartPos - g_staticParams.enzymeInformation.iSearchEnzyme2OffSet])
            && !strchr(g_staticParams.enzymeInformation.szSearchEnzyme2NoBreakAA, szProteinSeq[iStartPos - g_staticParams.enzymeInformation.iSearchEnzyme2OffSet + 1]);
      }
                                        ...
                                        int iEndPos)
{
   if (!g_staticParams.options.bNoEnzymeSelected)
   {
      ...
      if (!bEndCleavage && g_staticParams.enzymeInformation.szSearchEnzyme2Name[0] != '\0') // if second enzyme specified
      {
         bEndCleavage = strchr(g_staticParams.enzymeInformation.szSearchEnzyme2BreakAA, szProteinSeq[iEndPos - g_staticParams.enzymeInformation.iSearchEnzyme2OffSet + 1])
            && !strchr(g_staticParams.enzymeInformation.szSearchEnzyme2NoBreakAA, szProteinSeq[iEndPos - g_staticParams.enzymeInformation.iSearchEnzyme2OffSet + 2]);
      }
...
// Comet-PTM end
{
   ...
   unsigned int (*p_uiBinnedIonMassesDelta)[MAX_FRAGMENT_CHARGE + 1][9][MAX_PEPTIDE_LEN]; // Comet-PTM

   // Point to right set of arrays depending on target or decoy search.
   if (bDecoyPep)
   {
      p_uiBinnedIonMasses = &_uiBinnedIonMassesDecoy;
      p_uiBinnedIonMassesDelta = &_uiBinnedIonMassesDelta;  // Comet-PTM
   }
   else
   {
   ...
   // Comet-PTM: store all the fragment Xcorrs and DeltaXcorrs in this vector to calculate where the modification most likely is
   std::vector<std::vector<std::vector<std::vector<double> > > > vctXcorrs(pQuery->_spectrumInfoInternal.iMaxFragCharge,   // per charge
         std::vector<std::vector<std::vector<double> > >(g_staticParams.ionInformation.iNumIonSeriesUsed,                 // per series
               std::vector<std::vector<double> >(3, std::vector<double>(iLenPeptideMinus1, 0))));   // one for Xcorr, one for DeltaXcorr
         ...
         if (bDeltaXcorrSearch) // Comet-PTM
         {
            ...
            for (ctLen=0; ctLen<iLenPeptideMinus1; ctLen++)
            {
               if (bDeltaXcorrSearch) // Comet-PTM
               {
                  ...
                  if (xDelta < pQuery->iFastXcorrData && ppSparseFastXcorrData[xDelta] != NULL)
                  {
                     yDelta = binDelta - (xDelta*SPARSE_MATRIX_SIZE);
                     vctXcorrs[ctCharge-1][ctIonSeries][DELTA_XCORR_ID][ctLen] = ppSparseFastXcorrData[xDelta][yDelta];
                  }
               }
            }
         }
...
// Comet-PTM start: make this a separate function
   std::vector<double> vctXcorrAtPos(iLenPeptideMinus1, 0);
   ...
   if (bDeltaXcorrSearch && iLenPeptideMinus1 > 0)
   {
            ...
            for (ctIonSeries = 0; ctIonSeries < g_staticParams.ionInformation.iNumIonSeriesUsed; ctIonSeries++)
            {
               ...
               if (iWhichIonSeries == ION_SERIES_B)   // b series
               {
                  if (j - i > 0)
                  {
                     for (ctCharge = 1; ctCharge <= pQuery->_spectrumInfoInternal.iMaxFragCharge; ctCharge++)
                     {
                        vctXcorrAtPos[j] += vctXcorrs[ctCharge - 1][ctIonSeries][XCORR_ID][i];   // Xcorr
                        if (j == 0) dNonModXcorr += vctXcorrs[ctCharge - 1][ctIonSeries][XCORR_ID][i];
                     }
                  }
                  else
                  {
                     for (ctCharge = 1; ctCharge <= pQuery->_spectrumInfoInternal.iMaxFragCharge; ctCharge++)
                     {
                        vctXcorrAtPos[j] += vctXcorrs[ctCharge - 1][ctIonSeries][DELTA_XCORR_ID][i];    // deltaXcorr
                        if (j == 0) dNonModXcorr += vctXcorrs[ctCharge - 1][ctIonSeries][XCORR_ID][i];
                     }
                  }
               }
      ...
      best = std::max_element(vctXcorrAtPos.begin(), vctXcorrAtPos.end());
      bestPos = std::distance(vctXcorrAtPos.begin(), best);

      for (ctIonSeries = 0; ctIonSeries < g_staticParams.ionInformation.iNumIonSeriesUsed; ctIonSeries++)
      {
         iWhichIonSeries = g_staticParams.ionInformation.piSelectedIonSeries[ctIonSeries];

         // found best, now mark the vctBitMods to display it
         if (iWhichIonSeries == ION_SERIES_B)
         {
            for (int i = bestPos; i < iLenPeptideMinus1; ++i)   // b series
            {
               for (ctCharge = 1; ctCharge <= pQuery->_spectrumInfoInternal.iMaxFragCharge; ctCharge++)
               {
                  // DeltaXcorr > Xcorr
                  if (vctXcorrs[ctCharge - 1][ctIonSeries][DELTA_XCORR_ID][i] > vctXcorrs[ctCharge - 1][ctIonSeries][XCORR_ID][i])
                  {
                     vctBitMods[ctIonSeries][i] = 1;
                  }
               }
            }
         }
         ...
         {
            for (int i = iLenPeptideMinus1 - bestPos; i < iLenPeptideMinus1; ++i)   // y series
            {
               for (ctCharge = 1; ctCharge <= pQuery->_spectrumInfoInternal.iMaxFragCharge; ctCharge++)
               {
                  // DeltaXcorr > Xcorr
                  if (vctXcorrs[ctCharge - 1][ctIonSeries][DELTA_XCORR_ID][i] > vctXcorrs[ctCharge - 1][ctIonSeries][XCORR_ID][i])
                  {
                     vctBitMods[ctIonSeries][iLenPeptideMinus1 - i] = 1;
                  }
               }
            }
         }
...
// Comet-PTM end
   ...
   if (dNonModXcorr < XCORR_CUTOFF)
      dNonModXcorr = XCORR_CUTOFF;
      else
      dNonModXcorr *= dXcorrScale;
...
//    char bbb[256];
//    if (dXcorr > 1.030 && dXcorr < 1.031 && dDeltaXcorrMass > 328.220 && dDeltaXcorrMass < 328.221)
//    {
//          sprintf(bbb, " Xcorr: %f Deltamass: %f ", dXcorr, dDeltaXcorrMass);
//          logout(bbb);
//          logout(szProteinSeq);
//          fflush(stdout);
//    }
   ...
   if (bDecoyPep && g_staticParams.options.iDecoySearch==2)
   {
      if (dXcorr > pQuery->fLowestDecoyXcorrScore)
      {
         ...
         {
            g_pvQuery.at(iWhichQuery)->pbDuplFragment = pbDuplFragment; // TODO
            StorePeptide(iWhichQuery, iStartResidue, iStartPos, iEndPos, bFoundVariableMod, szProteinSeq,
                  dCalcPepMass, dXcorr, dNonModXcorr, bDecoyPep,  piVarModSites, dbe, pbDuplFragment, vctXcorrs);
         }
      }
   }
   else
...
// Comet-PTM start
                               bool *pbDuplFragment,
                               std::vector<std::vector<std::vector<std::vector<double> > > > vctXcorrs,
                               bool bDeltaXcorrSearch /*= false*/,
                               double dDeltaXcorrMass /*= 0.0*/,
                               std::vector<std::bitset<MAX_PEPTIDE_LEN> > vctBitMods,
...
// Comet-PTM end
{
   ...
   if (g_staticParams.options.iDecoySearch==2 && bDecoyPep)  // store separate decoys
   {
      ...
      if (g_staticParams.options.bUseXcorrCorr)
      {
          ...
          {
              dXcorrCorr = log10((float)dXcorr) / log10(2*(float)iLenPeptide);
          }
          else // R = 1.22
          {
              double dXcorr_R = dXcorr/1.22;
              dXcorrCorr = log10((float)dXcorr_R) / log10(2*(float)iLenPeptide);
          }
      ...
      {
         pQuery->_pDecoys[siLowestDecoySpScoreIndex].iModPos = iModPos;
         pQuery->_pDecoys[siLowestDecoySpScoreIndex].iStartPos = iStartPos;
         pQuery->_pDecoys[siLowestDecoySpScoreIndex].iEndPos = iEndPos;
         pQuery->_pDecoys[siLowestDecoySpScoreIndex].bDoRecom = 0;
         pQuery->_pResults[siLowestDecoySpScoreIndex].vcProteinSeq.clear();
         for (i=0; i<strlen(szProteinSeq); i++)
             pQuery->_pResults[siLowestDecoySpScoreIndex].vcProteinSeq.push_back(szProteinSeq[i]);
         pQuery->_pDecoys[siLowestDecoySpScoreIndex].vctXcorrs = vctXcorrs;
      }
   ...
   else
   {
      ...
      if (bDeltaXcorrSearch)  // Comet-PTM
      {
         char szTemp[MAX_PEPTIDE_LEN * 2];
         memcpy(szTemp, szProteinSeq + iStartPos, iLenPeptide);
         szTemp[iLenPeptide] = '\0';

         std::string sTempYSeries(szTemp);
         std::string sTempBSeries(szTemp);
         int iWhichIonSeries;

         for (int j = 0; j < 2; ++j)
         {
            iWhichIonSeries = g_staticParams.ionInformation.piSelectedIonSeries[j];

            int iDeltaAAs = 0;
            if (vctBitMods[j].any())
            {
               for (size_t i = 0; i < vctBitMods[j].size(); ++i)
               {
                  if (vctBitMods[j][i])
                  {
                     if (iWhichIonSeries == ION_SERIES_B)
                        sTempBSeries.insert(i+1 + iDeltaAAs, "<");
                     else if(iWhichIonSeries == ION_SERIES_Y)
                        sTempYSeries.insert(i + iDeltaAAs, ">");
                     ++iDeltaAAs;
                  }
               }
            }
         ...
         strcpy(pQuery->_pResults[siLowestSpScoreIndex].szDeltaBSeries, const_cast<char*>(sTempBSeries.c_str()));
         strcpy(pQuery->_pResults[siLowestSpScoreIndex].szDeltaYSeries, const_cast<char*>(sTempYSeries.c_str()));

         pQuery->_pResults[siLowestSpScoreIndex].dDeltaXcorrMass = dDeltaXcorrMass;
         //pQuery->_pResults[siLowestSpScoreIndex].sDeltaJumps = sDeltaJumps;
      }
      else
      {
         pQuery->_pResults[siLowestSpScoreIndex].dDeltaXcorrMass = 0.0;
         //pQuery->_pResults[siLowestSpScoreIndex].sDeltaJumps = "";
      }
      ...
      {
         pQuery->_pResults[siLowestSpScoreIndex].iModPos = iModPos;
         pQuery->_pResults[siLowestSpScoreIndex].iStartPos = iStartPos;
         pQuery->_pResults[siLowestSpScoreIndex].iEndPos = iEndPos;
         pQuery->_pResults[siLowestSpScoreIndex].bDoRecom = 0;
         pQuery->_pResults[siLowestSpScoreIndex].vcProteinSeq.clear();
         for (i=0; i<strlen(szProteinSeq); i++)
             pQuery->_pResults[siLowestSpScoreIndex].vcProteinSeq.push_back(szProteinSeq[i]);
         pQuery->_pResults[siLowestSpScoreIndex].vctXcorrs = vctXcorrs;
      }
   ...
   if (g_staticParams.options.iDecoySearch == 2 && bDecoyPep)
   {
      for (i=0; i<g_staticParams.options.iNumStored; i++)
      {
         ...
         if (iLenMinus1 == pQuery->_pDecoys[i].iLenPeptide && isEqual(dCalcPepMass, pQuery->_pDecoys[i].dPepMass))
         {
            ...
            // If bIsDuplicate & deltaX search, check delta_mod to see if peptide already stored.
            if (bIsDuplicate && bDeltaXcorrSearch)  // Comet-PTM
            {
               int flooring = 1e7;
               if (floor(pQuery->_pResults[i].dDeltaXcorrMass * flooring) == floor(dDeltaXcorrMass * flooring))
                  bIsDuplicate = 1;
               else
                  bIsDuplicate = 0;
            }
...
//FIX: check if piVarModSites index is correct
         ...
         XcorrScore(szProteinSeq, _varModInfo.iStartPos, _varModInfo.iEndPos, _varModInfo.iStartPos, _varModInfo.iEndPos,
               true, dCalcPepMass, false, iWhichQuery, iLenPeptide, piVarModSites, dbe, pbDuplFragment);

         if (g_staticParams.options.iDecoySearch)
         {
            XcorrScore(szDecoyPeptide, _varModInfo.iStartPos, _varModInfo.iEndPos, 1, iLenPeptide, true, 
                  dCalcPepMass, true, iWhichQuery, iLenPeptide, piVarModSitesDecoy, dbe, pbDuplFragment);
         }
      }
...
// Comet-PTM start
void CometSearch::DeltaXcorrSearch(const double& dDeltaXcorrMass,
                                   ...
                                   int iJump /*= 0*/)
{
   bool bDeltaXcorrSearch = g_staticParams.options.bUseDeltaXcorr &&
      ((dDeltaXcorrMass > -g_staticParams.options.dDeltaOuterTol && dDeltaXcorrMass < -g_staticParams.options.dDeltaInnerTol) ||
       (dDeltaXcorrMass > g_staticParams.options.dDeltaInnerTol && dDeltaXcorrMass < g_staticParams.options.dDeltaOuterTol));
   ...
   {       
      ...
      for (i = iStartPos; i < iEndPos; i++)
      {
         ...
         double dBionDelta = dBion + dDeltaXcorrMass;
         if (dBionDelta >= 0.0)
            _pdAAforwardDelta[iPos] = dBionDelta;
         else
            _pdAAforwardDelta[iPos] = 0.0;

         dYion += g_staticParams.massUtility.pdAAMassFragment[(int)szProteinSeq[iEndPos - iPos]];
         double dYionDelta = dYion + dDeltaXcorrMass;
         if (dYionDelta >= 0.0)
            _pdAAreverseDelta[iPos] = dYionDelta;
         else
            _pdAAreverseDelta[iPos] = 0.0;
      }
      ...
      for (int ctCharge = 1; ctCharge <= g_massRange.iMaxFragmentCharge; ctCharge++)
      {
         for (int ctIonSeries = 0; ctIonSeries < g_staticParams.ionInformation.iNumIonSeriesUsed; ctIonSeries++)
         {
            ...
            {
               pbDuplFragment[BIN(GetFragmentIonMass(iWhichIonSeries, ctLen, ctCharge, _pdAAforwardDelta, _pdAAreverseDelta))] = false;
            }
         }
      }
      ...
      for (int ctCharge = 1; ctCharge <= g_massRange.iMaxFragmentCharge; ctCharge++)
      {
         for (int ctIonSeries = 0; ctIonSeries < g_staticParams.ionInformation.iNumIonSeriesUsed; ctIonSeries++)
         {
            ...
            for (int ctLen = 0; ctLen < iLenMinus1; ctLen++)
            {
               int iValDelta = BIN(GetFragmentIonMass(iWhichIonSeries, ctLen, ctCharge, _pdAAforwardDelta, _pdAAreverseDelta));

               if (pbDuplFragment[iValDelta] == false)
               {
                  _uiBinnedIonMassesDelta[ctCharge][ctIonSeries][ctLen] = iValDelta;
                  pbDuplFragment[iValDelta] = true;
               }
               else
                  _uiBinnedIonMassesDelta[ctCharge][ctIonSeries][ctLen] = 0;
            }
         }
      }
      ...
      if (!g_staticParams.variableModParameters.bRequireVarMod)
      {
         ...
         {
            char buffer[2];
            sprintf(buffer, "%d", iJump);
            sJump += buffer;
         }

         // XcorrScore now takes into account deltaXcorr
         XcorrScore(szProteinSeq, iStartPos, iEndPos, iStartPos, iEndPos, false, dCalcPepMass, false,
               iWhichQuery, iLenPeptide, piVarModSites, dbe, pbDuplFragment, bDeltaXcorrSearch, dDeltaXcorrMass, sJump);
      }
