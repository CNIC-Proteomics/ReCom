...
//          logout("\n||rescored||");
//          fflush(stdout);
         CometRescore::RescorePeptides(iQueryIndex);
//          logout("||rescored||");
//          fflush(stdout);
      }
...
void CometPostAnalysis::AnalyzeSP(int i)
{
   ...
   {
       int iCandNumber = g_staticParams.options.iCandNumber;
       for (int ii=0; ii<iCandNumber; ii++)
          pQuery->_pResults[ii].bDoRecom = 1;
   }
