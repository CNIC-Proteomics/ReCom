...
void Usage(char *pszCmd)
{
   ...
   logout("\n");
   sprintf(szErrorMsg, "ReCom (Comet version \"%s\")\n %s\n", comet_version, copyright);
   logout(szErrorMsg);
   logout("\n");
   sprintf(szErrorMsg, " ReCom usage:  %s [options] <input_files>\n", pszCmd);
   logout(szErrorMsg);
   logout("\n");
   logout(" Supported input formats include mzXML, mzML, Thermo raw, mgf, and ms2 variants (cms2, bms2, ms2)\n");

   logout("\n");
   logout("       options:  -p         to print out a recom.params file (named recom.params.new)\n");
   logout("                 -P<params> to specify an alternate parameters file (default recom.params)\n");
...
// Reads recom.params parameter file.
      ...
      ICometSearchManager *pSearchMgr)
{
   ...
   while (!feof(fp))
   {
      ...
      {
         if (!strncmp(szParamBuf, "# recom_version ", 16))
         {
            char szRev1[12],
                 szRev2[12];
...
// Comet-PTM start
            ...
            else if (!strcmp(szParamName, "use_delta_closest"))
            {
                sscanf(szParamVal, "%d", &iIntParam);
                szParamStringVal[0] = '\0';
                sprintf(szParamStringVal, "%d", iIntParam);
                pSearchMgr->SetParam("use_delta_closest", szParamStringVal, iIntParam);
            }
            else if (!strcmp(szParamName, "deltamass_peaks"))
            {
                char szDeltaPeaksPath[512];
                std::vector<double> dDeltaPeaks;

                // Support parsing a string from params file that
                // includes spaces in the path.
                // Remove white spaces at beginning/end of szParamVal
                int iLen = strlen(szParamVal);
                char *szTrimmed = szParamVal;
                ...
                memmove(szParamVal, szTrimmed, iLen + 1);
                strcpy(szDeltaPeaksPath, szParamVal);

                ifstream infile;
                string temp;
                infile.open(szDeltaPeaksPath);
                while (!infile.eof())
                {
                    infile >> temp;
                    dDeltaPeaks.push_back(std::stod(temp));
                }
                infile.close();

                pSearchMgr->SetParam("deltamass_peaks", szDeltaPeaksPath, dDeltaPeaks);
            }
            else if (!strcmp(szParamName, "deltamass_peaks_tolerance"))
            {
                sscanf(szParamVal, "%lf", &dDoubleParam);
                szParamStringVal[0] = '\0';
                sprintf(szParamStringVal, "%lf", dDoubleParam);
                pSearchMgr->SetParam("deltamass_peaks_tolerance", szParamStringVal, dDoubleParam);
            }
            else if (!strcmp(szParamName, "print_xcorr_profiles"))
            {
                sscanf(szParamVal, "%d", &iIntParam);
                szParamStringVal[0] = '\0';
                sprintf(szParamStringVal, "%d", iIntParam);
                pSearchMgr->SetParam("print_xcorr_profiles", szParamStringVal, iIntParam);
            }
            else if (!strcmp(szParamName, "cand_number"))
            {
                sscanf(szParamVal, "%d", &iIntParam);
                szParamStringVal[0] = '\0';
                sprintf(szParamStringVal, "%d", iIntParam);
                pSearchMgr->SetParam("cand_number", szParamStringVal, iIntParam);
            }
            else if (!strcmp(szParamName, "use_xcorr_corr"))
            {
                sscanf(szParamVal, "%d", &iIntParam);
                szParamStringVal[0] = '\0';
                sprintf(szParamStringVal, "%d", iIntParam);
                pSearchMgr->SetParam("use_xcorr_corr", szParamStringVal, iIntParam);
            }
// Comet-PTM end
            else
            {
                    ...
                    ICometSearchManager *pSearchMgr)
{
   ...
   strcpy(szParamsFile, "recom.params");
   ...
   // Loads search parameters from recom.params file. This has to happen
...
void PrintParams(void)
{
   ...
   if ( (fp=fopen("recom.params.new", "w"))==NULL)
   {
      char szErrorMsg[256];
      sprintf(szErrorMsg, "\n ReCom (Comet version %s)\n\n", comet_version);
      sprintf(szErrorMsg+strlen(szErrorMsg), " Error - cannot write file recom.params.new\n");
      logerr(szErrorMsg);
      exit(1);
   }

   fprintf(fp, "# recom_version %s\n\
...
#\n\
use_delta_xcorr = 0                    # 0=no (default), 1=yes\n\
delta_outer_tolerance = 320            # ignored if use_delta_xcorr 0, default 320 \n\
delta_inner_tolerance = 0.8            # ignored if use_delta_xcorr 0, default 0.8 \n\
use_delta_back_jumps = 0               # 0=no (default), 1=yes\n\
use_delta_forward_jumps = 0            # 0=no (default), 1=yes\n\
dont_calc_pseudo_non_mod = 1\n\
use_delta_closest = 0                  # 0=no (default), 1=yes\n\
deltamass_peaks = /some/path/p.txt     # if use_delta_closest = 1, path must be set here to peaks file\n\
deltamass_peaks_tolerance = 1          # +- Da tolerance for matching experimental masses to the list, default 1\n\
cand_number = 1                        # number of candidates to rescore for each identification, default 1\n\
use_xcorr_corr = 0                     # use corrected xcorr score, 0=no (default), 1=yes\n\
\n\
#\n\
